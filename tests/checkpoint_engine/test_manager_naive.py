...
"""CPU-only tests for colocated checkpoint weight synchronization."""
...
def test_colocated_sync_streams_generator_without_checkpoint_engine():
...
def test_colocated_lora_sync_reuses_peft_config_and_sends_only_incremental_weights():
...
def test_colocated_merge_sync_generates_a_full_weight_set_each_time():
...
def test_colocated_failure_does_not_mark_base_sync_done():
    model = _Model([('q_proj.weight', 'base')])
...
def test_colocated_weight_generator_failure_does_not_mark_base_sync_done():
...
def test_mixed_deployment_shape_fails_at_initialization():
    ...
    with pytest.raises(ValueError, match='same deployment mode'):
        CheckpointEngineManager(model, sampler, platform='CPU')


@pytest.mark.parametrize('provide_weights', [True, False])
def test_sampler_receive_weights_selects_direct_or_checkpoint_stream(provide_weights):
    ...
    vLLMSampler.receive_weights.__wrapped__(sampler, weights=direct_weights)
    ...
    assert sampler.engine.invalidated is True
    assert len(checkpoint_engine_calls) == (0 if provide_weights else 1)


def test_ray_actor_sync_keeps_checkpoint_engine_lifecycle(monkeypatch):
    events = []
    ...
    monkeypatch.setattr(CheckpointEngineManager, 'decide_backend_engine', staticmethod(lambda platform=None: _Backend))

    manager = CheckpointEngineManager(model, sampler, platform='GPU')
    manager.sync_weights()

    assert manager._naive is False
    ...
    receive_kwargs = next(event[1] for event in events if isinstance(event, tuple)
                          and event[0] == 'receive_submitted')
