...
from twinkle.server.state.config_signature import SignatureMismatchPolicy, compute_signature, validate_config_signature
...
async def test_different_config_warn_policy():
    ...
    result = await validate_config_signature(backend, config_v2, policy=SignatureMismatchPolicy.WARN)
...
async def test_different_config_clear_policy():
    ...
    result = await validate_config_signature(backend, config_v2, policy=SignatureMismatchPolicy.CLEAR)
    assert result is False
...
async def test_different_config_abort_policy():
    ...
    with pytest.raises(ConfigMismatchError):
        await validate_config_signature(backend, config_v2, policy=SignatureMismatchPolicy.ABORT)
