...
class GaLoreAdamW8bit(Optimizer2State):
    """8-bit AdamW with GaLore low-rank gradient projection.

    A param group carrying the `rank`/`update_proj_gap`/`scale`/`proj_type` keys is projected onto a
    low-rank subspace before the moments are updated, other param groups behave like a plain AdamW8bit.
    """
    ...
    def __init__(self,
        ...
        try:
            ...
            raise ImportError('GaLoreAdamW8bit requires bitsandbytes, please install it by `pip install bitsandbytes`.') from e
