...
def npu_builtin(model: nn.Module | None = None) -> dict[Any, dict[str, Any]]:
    ...
    if is_npu_platform:
        ...
        if exists('flash-linear-attention'):
            apply_qwen3_5_fla(model)
        else:
            logger.warning('[NPU] [FLA] flash-linear-attention is not installed; '
                           'FLA patch for Qwen3.5 requires the fla package (which ships '
                           'its own Ascend-backend dispatch). '
