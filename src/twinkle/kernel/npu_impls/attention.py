...
"""SDPA + DeepSeek-V4 attention forwards with Ascend NPU compatibility fixes."""
...
# the ``npu_builtin()`` bundle (see ``builtin._add_deepseek_v4_entries``).
# ---------------------------------------------------------------------------


def _npu_dsv4_sparse_attn_shared_kv(query,
                                    ori_kv,
                                    cmp_kv,
                                    cmp_sparse_indices,
                                    sinks,
    ...
    cmp_sparse_indices = None if cmp_ratio != 4 or cmp_sparse_indices is None else cmp_sparse_indices.unsqueeze(
        2).contiguous()
...
def npu_dsv4_attention_forward(
    ...
    **kwargs,
    ...
    Falls back to the standard HF attention interface when mindspeed is
    unavailable. Expects ``self.compressor(...)`` to return a 3-tuple
    ``(compressed_kv, block_bias, top_k_indices)`` (see
    ...
    if self.layer_type == 'sliding_attention':
        ...
        if compressed_kv is not None and compressed_kv.shape[2] > 0:
            ...
            if top_k_indices is not None:
                # The indexer marks invalid (causal-future) positions with -1.
                # The NPU SAS kernel's backward scatter-gradients at -1 →
                # unsigned wrap-around → "DDR address out of range".  Clamp
                # -1 to 0 (a valid index) so the kernel never receives a
                # negative index.  The attention weight for block 0 on early
                # queries is negligible relative to the sliding-window KV.
    ...
    try:
        attn_output = _npu_dsv4_sparse_attn_shared_kv(
...
def npu_dsv4_indexer_forward(
    ...
    Falls back to a pure-torch top-k implementation when mindspeed is
    unavailable.
    """
...
def npu_dsv4_csa_compressor_forward(
    ...
    (``store_compression_weights`` appends kv/gate on every call). Under
