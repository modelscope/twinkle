...
class GmmFunction(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, group_list: torch.Tensor, weight_ekn: torch.Tensor):
        ...
        outputs = torch_npu.npu_grouped_matmul(
            [x], [weight_ekn],
            group_list=group_list, group_type=0, split_item=2, group_list_type=1,
        )
        return outputs[0]
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch_npu.npu_grouped_matmul(
            ...
            bias=None, group_list=group_list, group_type=0, split_item=2, group_list_type=1,
        )[0]
        grad_weight = torch_npu.npu_grouped_matmul(
            [x.transpose(0, 1)], [grad_output],
            bias=None, group_list=group_list, group_type=2, split_item=3, group_list_type=1,
        )[0]
...
def npu_sdpa_attention_forward(module, query, key, value, attention_mask,
