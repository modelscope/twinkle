...
from twinkle.server.telemetry.provider import TelemetryConfig
...
class ServerConfig(BaseModel):
    ...
    def _validate_cross_field(self) -> 'ServerConfig':
        ...
        if self.persistence.mode == 'redis' and not self.persistence.redis_url:
            raise ValueError(
                "persistence.redis_url is required when persistence.mode == 'redis'",
            )
        # R7.2: file mode requires file_path
        if self.persistence.mode == 'file' and not self.persistence.file_path:
            raise ValueError(
                "persistence.file_path is required when persistence.mode == 'file'",
            )
        return self
