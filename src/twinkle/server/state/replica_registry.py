...
in the configured :class:`StateBackend`, so two workers on the same shared
backend (Redis in multi-node, MemoryBackend in single-node) see one consistent
view of the cluster's capacity.
