...
class vLLMSampler(Sampler, CheckpointEngineMixin):
    ...
    def sample_stream(
        ...
        Bridges the background event loop where the vLLM engine runs to the
        caller's event loop using a thread-safe queue.
        ...
        async def _produce():
            try:
                async for delta_text, finish_reason in self.engine.generate_stream(
                    prompt=prompt,
                    sampling_params=sampling_params,
                    lora_request=lora_request,
                ):
                    q.put((delta_text, finish_reason))
            except Exception as e:
                q.put(e)
            finally:
                q.put(_SENTINEL)

        asyncio.run_coroutine_threadsafe(_produce(), self._async_loop)
