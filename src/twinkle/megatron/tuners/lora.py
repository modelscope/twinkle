...
class LoraParallelLinear(MegatronModule, LoraLayer):
    ...
    def update_layer(
        ...
        **kwargs
        ...
        # Disable overlap for LoRA layers
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any):
        ...
        if not isinstance(self.base_layer, TopKRouter) and not self.disable_adapters and not self.merged:
            ...
            for active_adapter in self.active_adapters:
                ...
                lora_result = lora_A(dropout(x), *args, **kwargs) if isinstance(lora_A, TEGroupedLinear) else lora_A(dropout(x))
                if isinstance(lora_result, tuple):
                    lora_result = lora_result[0]
                    
                lora_result = lora_B(lora_result, *args, **kwargs) if isinstance(lora_B, TEGroupedLinear) else lora_B(lora_result)
                if isinstance(lora_result, tuple):
                    lora_result = lora_result[0]
    ...
    def sharded_state_dict(
        ...
        if prefix.endswith('linear_fc1.'):
            if isinstance(self.base_layer, TEGroupedLinear) and self.config.gated_linear_unit:
                num_global_experts = (
                    parallel_state.get_expert_model_parallel_world_size() * self.base_layer.num_gemms
                )
    ...
    def get_delta_weights(self, adapter: str) -> List[torch.Tensor]:
        ...
        for i in range(len(weight_B)):
            output_tensor.append(
                transpose(weight_B[i] @ weight_A[i], self.fan_in_fan_out) * self.scaling[adapter]
            )
