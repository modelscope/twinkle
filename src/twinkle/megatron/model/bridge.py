...
class SafetensorLoader:
    ...
    def _load_index(self):
        ...
        if self.is_peft_format:
            adapter_file = os.path.join(self.model_dir, 'adapter_model.safetensors')
        ...
        if os.path.exists(index_file):
            ...
            if os.path.exists(single_file):
                ...
                files = glob.glob(os.path.join(self.model_dir, '*.safetensors'))
    ...
    def _get_handle(self, filepath: str):
        ...
        if filepath not in self._handles:
            self._handles[filepath] = safe_open(filepath, framework='pt', device='cpu')
...
class StreamingSafetensorSaver:
    ...
    def __init__(self, save_dir: str, max_shard_size: str = '5GB', is_peft_format: bool = False):
    ...
    def finalize(self):
        ...
        if total_shards > 1:
            ...
            with open(os.path.join(self.save_dir, 'model.safetensors.index.json'), 'w') as f:
...
class TwinkleGPTBridge:
    ...
    def __init__(self, config: BridgeConfig, hf_config: Any = None, disable_tqdm: bool = False):
    ...
    def _get_tp_split_dim(self, mg_key: Optional[str]) -> Optional[int]:
        ...
        dim0_keys = {
            'word_embeddings', 'linear_qkv', 'output_layer',
            'linear_q_proj', 'linear_q_up_proj', 'linear_kv_up_proj',
            'eh_proj',  # MTP
        }
    ...
    def _split_tp(self, tensor: torch.Tensor, tp_dim: Optional[int], is_expert: bool = False) -> torch.Tensor:
    ...
    def _all_gather_tp(self, tensor: Optional[torch.Tensor], tp_dim: Optional[int], 
    ...
    def _load_embedding(self, mg_model, loader: SafetensorLoader):
        ...
        self._set_weight(embed_module.weight, hf_weight, 'word_embeddings.weight')
    ...
    def _load_output_layer(self, mg_model, loader: SafetensorLoader):
        ...
        self._set_weight(output_module.weight, hf_weight, 'output_layer.weight')
    ...
    def _load_attention(self, mg_layer, loader: SafetensorLoader, layer_idx: int):
    ...
    def _load_mlp(self, mg_layer, loader: SafetensorLoader, layer_idx: int):
        ...
        try:
            ...
            try:
                gate_up_weight = loader.get_tensor(f'{prefix}gate_up_proj.weight')
                gate_up_weight = gate_up_weight.view(2, -1, gate_up_weight.shape[-1])
                self._set_weight(mg_mlp.linear_fc1.weight, gate_up_weight, 'linear_fc1.weight')
            except KeyError:
                pass
        ...
        try:
            ...
            self._set_weight(mg_mlp.linear_fc2.weight, down_weight, 'linear_fc2.weight')
        except KeyError:
            pass
    ...
    def _load_moe(self, mg_layer, loader: SafetensorLoader, layer_idx: int):
        ...
        if self.config.shared_expert_intermediate_size > 0:
            for shared_key in ['shared_expert', 'shared_experts', 'shared_mlp']:
                try:
                    gate_weight = loader.get_tensor(f'{prefix}{shared_key}.gate_proj.weight')
                    up_weight = loader.get_tensor(f'{prefix}{shared_key}.up_proj.weight')
                    down_weight = loader.get_tensor(f'{prefix}{shared_key}.down_proj.weight')
                    
                    shared_module = deep_getattr(mg_mlp, 'shared_experts')
                    if shared_module is not None:
                        fc1_weight = torch.stack([gate_weight, up_weight], dim=0)
                        self._set_weight(shared_module.linear_fc1.weight, fc1_weight, 'linear_fc1.weight')
                        self._set_weight(shared_module.linear_fc2.weight, down_weight, 'linear_fc2.weight')
                    break
                except KeyError:
                    continue
    ...
    def load_weights(
        ...
        with torch.no_grad():
            with SafetensorLoader(model_path, is_peft_format=is_peft_format) as loader:
                if is_peft_format:
    ...
    def _load_peft_weights(self, mg_model: nn.Module, loader: SafetensorLoader):
    ...
    def export_weights(
        ...
        with torch.no_grad():
            ...
            if not is_peft_format:
                # Export embedding
                if self.pp_size <= 1 or self.pp_rank == 0:
                    embed = deep_getattr(mg_model, 'embedding.word_embeddings.weight')
                    if embed is not None:
                        weight, _ = self._get_weight(embed.data, 'word_embeddings.weight')
            ...
            for mg_layer in prog_bar:
                ...
                yield from self._export_layer(mg_layer, layer_idx, hf_prefix, is_peft_format)
            ...
            if not is_peft_format:
                ...
                if self.pp_size <= 1 or self.pp_rank == self.pp_size - 1:
                    ...
                    if output is not None:
                        weight, _ = self._get_weight(output.data, 'output_layer.weight')
                        if weight is not None:
                            weight = weight[:self.config.vocab_size]
    ...
    def _export_layer(
        ...
        if not is_peft_format:
            # Export QKV
            qkv_weight, _ = self._get_weight(mg_attn.linear_qkv.weight.data, 'linear_qkv.weight')
            if qkv_weight is not None:
                qkv_weight = qkv_weight.reshape(num_kv_heads, -1, hidden_size)
                yield f'{prefix}self_attn.q_proj.weight', qkv_weight[:, :q_dim, :].reshape(-1, hidden_size).clone()
                yield f'{prefix}self_attn.k_proj.weight', qkv_weight[:, q_dim:q_dim+kv_dim, :].reshape(-1, hidden_size).clone()
                yield f'{prefix}self_attn.v_proj.weight', qkv_weight[:, -kv_dim:, :].reshape(-1, hidden_size).clone()
            
            # Export O
            o_weight, _ = self._get_weight(mg_attn.linear_proj.weight.data, 'linear_proj.weight')
            ...
            # Export MLP
            fc1_weight, _ = self._get_weight(mg_mlp.linear_fc1.weight.data, 'linear_fc1.weight')
            ...
            fc2_weight, _ = self._get_weight(mg_mlp.linear_fc2.weight.data, 'linear_fc2.weight')
    ...
    def _export_lora_layer(
        ...
        if isinstance(mg_attn.linear_qkv, LoraParallelLinear):
            lora_A = deep_getattr(mg_attn.linear_qkv, f'lora_A.{self._adapter_name}.weight')
            lora_B = deep_getattr(mg_attn.linear_qkv, f'lora_B.{self._adapter_name}.weight')
            
            if lora_A is not None and lora_B is not None:
                lora_A, _ = self._get_weight(lora_A.data, 'linear_qkv.lora_A.weight')
                lora_B, _ = self._get_weight(lora_B.data, 'linear_qkv.lora_B.weight')
                ...
                if lora_A is not None:
                    ...
                    yield f'{prefix}self_attn.q_proj.lora_B.weight', lora_B[:, :q_dim, :].reshape(-1, lora_B.shape[-1]).clone()
                    yield f'{prefix}self_attn.k_proj.lora_B.weight', lora_B[:, q_dim:-head_dim, :].reshape(-1, lora_B.shape[-1]).clone()
                    yield f'{prefix}self_attn.v_proj.lora_B.weight', lora_B[:, -head_dim:, :].reshape(-1, lora_B.shape[-1]).clone()
        
        # O projection LoRA
        if isinstance(mg_attn.linear_proj, LoraParallelLinear):
            lora_A = deep_getattr(mg_attn.linear_proj, f'lora_A.{self._adapter_name}.weight')
            lora_B = deep_getattr(mg_attn.linear_proj, f'lora_B.{self._adapter_name}.weight')
            
            if lora_A is not None and lora_B is not None:
                lora_A, _ = self._get_weight(lora_A.data, 'linear_proj.lora_A.weight')
                lora_B, _ = self._get_weight(lora_B.data, 'linear_proj.lora_B.weight')
        ...
        # MLP LoRA
        if hasattr(mg_mlp, 'linear_fc1') and isinstance(mg_mlp.linear_fc1, LoraParallelLinear):
            lora_A = deep_getattr(mg_mlp.linear_fc1, f'lora_A.{self._adapter_name}.weight')
            lora_B = deep_getattr(mg_mlp.linear_fc1, f'lora_B.{self._adapter_name}.weight')
            
            if lora_A is not None and lora_B is not None:
                lora_A, _ = self._get_weight(lora_A.data, 'linear_fc1.lora_A.weight')
                lora_B, _ = self._get_weight(lora_B.data, 'linear_fc1.lora_B.weight')
                ...
                if lora_A is not None:
                    ...
                    yield f'{prefix}mlp.gate_proj.lora_B.weight', lora_B[0].clone()
                    yield f'{prefix}mlp.up_proj.lora_B.weight', lora_B[1].clone()
        
        if hasattr(mg_mlp, 'linear_fc2') and isinstance(mg_mlp.linear_fc2, LoraParallelLinear):
            lora_A = deep_getattr(mg_mlp.linear_fc2, f'lora_A.{self._adapter_name}.weight')
            lora_B = deep_getattr(mg_mlp.linear_fc2, f'lora_B.{self._adapter_name}.weight')
            
            if lora_A is not None and lora_B is not None:
                lora_A, _ = self._get_weight(lora_A.data, 'linear_fc2.lora_A.weight')
                lora_B, _ = self._get_weight(lora_B.data, 'linear_fc2.lora_B.weight')
    ...
    def save_weights(
        ...
        if should_write:
            ...
            if is_peft_format and hasattr(mg_models[0], 'peft_config'):
                peft_config = copy(mg_models[0].peft_config.get(self._adapter_name))
                if peft_config is not None:
                    peft_config.target_modules = list(self._peft_target_modules)
                    peft_config.modules_to_save = list(self._peft_modules_to_save)
