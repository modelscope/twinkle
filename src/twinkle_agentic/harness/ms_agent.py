...
class MsAgentHarness(AgentHarness):
    ...
    Args:
        ...
        freeze_system: if True (default, RL-safe), do not rewrite
            ...
            paths still run.
        append_only: if True (default, RL-safe), skip every step that would
            *rewrite* history rather than extend it -- memory condensation,
            context reassembly, task-notification injection, the post-tool
            runtime hooks. The multi-turn ledger trains on the exact token ids
            the sampler returned and keeps them by appending only; a harness
            that summarised the first twenty turns into a paragraph would move
            every logprob onto a token that no longer sits where it did. So the
            RL path keeps the ms-agent *message shape* (tool-result framing,
            assistant normalisation, the tool schema) and defers the rewriting
            features. Set False for a serving-style run that owns no ledger.
    ...
    def before_generate(self, trajectory: Trajectory) -> Trajectory:
        ...
        if self.auto_prepare:
            self.prepare()
        # append_only: history-rewriting is off, and before_generate only ever
        # rewrites (condense, reassemble, refresh the system prompt). Prepare so
        # the tool/memory runtime exists for the append-only hooks, then leave the
        # conversation exactly as the ledger banked it.
        if self.append_only:
            return trajectory
    ...
    def after_generate(
        ...
        if not self.append_only and getattr(self.agent, 'runtime', None) is not None and response.tool_calls:
    ...
    def after_tools(
        ...
        # append_only: the tool messages above are appends and stay; condensation
        # and the post-tool runtime hooks rewrite earlier turns, so the ledger
        # would reject them -- defer to a serving run.
