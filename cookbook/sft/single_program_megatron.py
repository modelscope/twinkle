...
def train():
    ...
    model.set_optimizer(optimizer_cls='default', lr=1e-5, adapter_name='default')
    model.set_lr_scheduler(scheduler_cls='default', lr_warmup_steps=10, lr_decay_steps=len(dataloader), adapter_name='default')
    logger.info(get_device_placement())
    logger.info(model.get_train_configs(adapter_name='default'))
    logger.info(f'Total steps: {len(dataloader)}')
    loss_metric = 99.0
    for step, batch in enumerate(dataloader):
        model.forward_backward(inputs=batch, micro_batch_size=1, adapter_name='default')
        outputs = model.forward_only(inputs=batch, adapter_name='default')
        model.clip_grad_and_step(adapter_name='default')
        if step % 5 == 0:
            logger.info(f'Current is step {step} of {len(dataloader)}, metric: {model.calculate_metric(is_training=True, adapter_name='default')}')
