"""Twinkle C/S training demo with dataset processing on the server.

Dataset, template encoding, and batching are executed by the remote Processor
service. Dataset and tokenizer/model paths must therefore be visible from every
Ray node on which a Processor Dataset actor may run.
"""
...
def build_remote_dataloader() -> DataLoader:
    # Do not use os.path.exists() here: it would only check the client machine.
    ...
    dataset.encode(timeout=3600, num_proc=8, load_from_cache_file=True)
    return DataLoader(
        dataset=dataset,
        batch_size=batch_size,
...
def train() -> None:
    ...
    dataloader = build_remote_dataloader()
