...
AGENT_RUNNER = args.challenger.agent_runner
...
if AGENT_CONFIG and AGENT_RUNNER not in ('external', 'harness'):
    raise SystemExit(f"--agent-runner must be 'external' or 'harness', got {AGENT_RUNNER!r}")
...
if AGENT_CONFIG and AGENT_RUNNER == 'external' and not AGENT_ENDPOINT_HOST:
    raise SystemExit('--agent-config with --agent-runner external needs --agent-endpoint-host: the '
                     'agent calls the policy from inside the sandbox, where loopback is the sandbox '
                     'itself. Give the address of this host that the sandbox can reach. (The '
                     "'harness' runner drives the loop locally and needs no endpoint.)")
...
def create_seeder(sampler, rollout_template, sampling_params) -> Optional[Seeder]:
    ...
    None when nothing is configured -- ``--num-keywords 0`` and no seed dataset --
    which the challenger reads as "propose from scratch", so a run can start
    without first spending rollouts inventing topics.
    """
    seeders: List[Seeder] = []
    ...
    # --num-keywords 0 turns the keyword pool off: no rollouts are spent inventing
    # topics before training, and with no seed dataset either the challenger just
    # proposes from scratch. Any positive count keeps the pool.
    if args.challenger.num_keywords > 0:
        seeders.append(
            KeywordSeeder(
...
def main():
    ...
    solver_rollout = None
    if AGENT_CONFIG and AGENT_RUNNER == 'external':
    ...
    elif AGENT_CONFIG and AGENT_RUNNER == 'harness':
        # The dependency-inverted twin of the branch above. The ms-agent harness
        # shapes messages and normalises tool turns, but the loop runs here and
        # generation goes through the local sampler, so only a forward tunnel to
        # the sandbox tools is needed -- no policy endpoint, no reverse route back
        # from the sandbox. Tools still execute through the env's own manager
        # (passed per call by the challenger); the harness only frames them. One
        # harness per concurrent slot, rebuilt between episodes so an agent's
        # memory never crosses tasks.
        solver_rollout = MultiTurnRollout(
            sampler,
            rollout_template,
            sampling_params=sampling_params,
            max_turns=MAX_TURNS,
            harness=HarnessLeases(lambda: MsAgentHarness(config=AGENT_CONFIG), NUM_ENVS),
        )
