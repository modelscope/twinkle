...
def create_dataset():
    ...
    dataset = Dataset(
        dataset_meta=DatasetMeta(
            dataset_id=args.dataset, subset_name=args.subset, split='train', data_slice=range(args.samples)))

    # Apply preprocessing
    dataset.dataset = dataset.dataset.map(preprocess_latex_ocr, batched=False, load_from_cache_file=False)

    # Filter out invalid samples
    dataset.dataset = dataset.dataset.filter(lambda x: len(x.get('messages', [])) > 0)
...
def train():
    ...
    model.add_adapter_to_model('vlm_lora', lora_config, gradient_accumulation_steps=GAS)
    ...
    for step, batch in enumerate(dataloader):
        ...
        if (step + 1) % GAS == 0:
            ...
            logger.info(
                f'Step {optimizer_step:3d} | loss: {avg_loss:.4f} | grad_norm: {grad_norm_value:.4f} | time: {avg_step_time:.3f}s'
            )
