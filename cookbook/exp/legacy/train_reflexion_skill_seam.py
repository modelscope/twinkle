...
def _load_seam_parquet(path: str) -> List[Dict[str, Any]]:
    ...
    ``reference_answer <- reward_model.ground_truth``. No shuffle/filter:
    ...
    for i, r in enumerate(rows):
        ...
        if not problem or ref is None:
            continue
        out.append({'problem': problem, 'reference_answer': str(ref),
                    'data_id': f"seam:{ei.get('split', '')}:{ei.get('index', i)}"})
    return out


def _load_records_from_seam(args: argparse.Namespace, seam_dir: str
                            ) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]],
                                       Dict[str, Tuple[str, float]], Dict[str, str], Dict[str, int]]:
    """Data entry that mirrors a SEAM run EXACTLY: read ``train.parquet``/``val.parquet`` from
    ``seam_dir`` in file order, use ``val`` as the eval holdout, take the first ``--n`` train rows
    (post ``--pool-offset``) with NO shuffle. ``--numeric-only``/``--eval-size``/internal shuffle
    are bypassed (the parquet is already the authoritative split)."""
    ...
    if args.xproblem_rubric:
        raise ValueError('--xproblem-rubric is unsupported with --seam-parquet-dir '
                         '(SEAM parquet carries no neighbour structure).')
    pool = _load_seam_parquet(tp)  # already SEAM-shuffled + truncated, in file order
    eval_records = [dict(r) for r in _load_seam_parquet(vp)]  # SEAM's exact val holdout
    pool_offset = max(0, int(getattr(args, 'pool_offset', 0) or 0))
    if pool_offset:
        if pool_offset >= len(pool):
            raise ValueError(f'--pool-offset ({pool_offset}) leaves no train records '
                             f'from SEAM train pool size {len(pool)}')
    ...
    stats = {'raw_loaded': len(pool) + len(eval_records), 'numeric_dropped': 0,
             'excluded_records': 0, 'pool_offset': pool_offset,
             'train_records': len(train_records), 'eval_records': len(eval_records),
             'source': 'seam_parquet', 'seam_parquet_dir': seam_dir}
...
def _load_records(args: argparse.Namespace
    ...
    if seam_dir:  # read SEAM parquet in file order, bypassing load/filter/shuffle/split
        return _load_records_from_seam(args, seam_dir)
