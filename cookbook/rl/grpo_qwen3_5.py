...
def create_gsm8k_dataset():
    ...
    dataset.set_template('Template', model_id=MODEL_ID, max_length=1024)
...
def main():
    device_groups = [
        DeviceGroup(name='model', ranks=list(range(4, 8)), device_type='GPU'),
        DeviceGroup(name='sampler', ranks=list(range(4)), device_type='GPU'),
    ]
    # tp=2, pp=2, ep=2 for model group (4 GPUs)
    # dp = world_size / (tp * pp) = 4 / (2 * 2) = 1
    model_mesh = DeviceMesh.from_sizes(world_size=MODEL_GPUS, dp_size=1, tp_size=2, pp_size=2, ep_size=2)
    sampler_mesh = DeviceMesh.from_sizes(world_size=SAMPLER_GPUS, dp_size=SAMPLER_GPUS)
    twinkle.initialize(mode='ray', nproc_per_node=NUM_GPUS, groups=device_groups, lazy_collect=False)

    lora_config = LoraConfig(target_modules='all-linear', r=8, lora_alpha=16, lora_dropout=0.05)
    ...
    model.save('grpo-qwen35-gsm8k-checkpoint')
