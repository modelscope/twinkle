"""AgentENV backend: one Firecracker microVM per trajectory.

Code runs in real CPython inside a microVM. Two properties shape the prompt
below, and both are the OPPOSITE of the OpenEnv backend:

  * Each ``run_python`` call is a FRESH process, so every snippet must be
    self-contained — nothing carries over between calls.
  * It is a full interpreter: the whole standard library works, decorators and
    ``unittest`` behave normally, and files can be written.
"""
import os
import textwrap
