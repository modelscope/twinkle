...
def _maybe_set_num_layers(config, num_layers: int | None) -> None:
...
def main():
    ...
    parser.add_argument('--dataset-id', type=str, default=os.environ.get('QWEN3_DATASET_ID', 'path/to/alpaca/dataset'))
    ...
    parser.add_argument('--ep-size', type=int, default=None, help='Enable EP when > 1')
    ...
    if os.path.exists(args.dataset_id):
        ...
        raw_dataset = load_dataset(file_type, data_files=args.dataset_id, split='train')
    else:
        raw_dataset = load_dataset(args.dataset_id, split='train')
    tokenized = raw_dataset.map(_tokenize_alpaca, remove_columns=raw_dataset.column_names)

    def _collate_fn(features):
        return {k: torch.tensor([f[k] for f in features], device=device) for k in features[0].keys()}
