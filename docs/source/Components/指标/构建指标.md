...
class Metric:
    ...
    # 由于microbatch的存在，输入到Metric的inputs可能是个List
...
指标无法通过Callable传入。因为它包含了`accumulate`和`calculate`两个部分，并需要支持`reset`来归零。指标的构造中会自动传入device_mesh和隶属于当前dp组的process_group，用以跨进程通讯。
