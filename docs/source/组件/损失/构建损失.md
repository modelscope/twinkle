...
损失的输入为模型的`InputFeature`，输出为模型标准`ModelOutput`，kwargs可以在模型的calculate_loss中传入。由于它是一个带有`__call__`方法的类，因此开发者也可以使用Callable:
...
你也可以将Loss上传到ModelScope/Hugging Face的Hub中，在使用时动态拉取：
