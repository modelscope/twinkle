...
- 安装 `triton-ascend`
- 安装 `flash-linear-attention` >= 0.5.2（fla 自带 Ascend 后端 dispatch `fla/backends/triton_ascend`，无需 MindSpeed 或厂商自有 Triton kernel）

```bash
pip install flash-linear-attention>=0.5.2
```

Twinkle 将 Qwen3.5 FLA 直接委托给 fla 原生算子（`fla.modules.convolution.causal_conv1d` 与 `fla.ops.gated_delta_rule.chunk_gated_delta_rule`）。在 NPU 上 fla 的 backend dispatch 会自动路由到 `triton_ascend` 后端。这与 ms-swift 的做法一致（`swift.model.npu_patch.mindspeed.patch_mindspeed_fla_gdn_implementation` 优先使用上游 fla 而非 MindSpeed 的 GDN 实现）。
...
> **注意**：如环境中安装了多个 fla 版本（例如旧的常规安装遮蔽了 editable 安装），请确保带 `triton_ascend` 后端的版本（>= 0.5.2）优先加载。可通过以下命令验证：
> ```python
> import fla; print(fla.__file__)  # 应指向 0.5.2+ 的安装
> from fla.ops.gated_delta_rule.backends import gdr_registry
> print([b.backend_type for b in gdr_registry._get_sorted_backends()])  # 应包含 'triton_ascend'
> ```
...
| `TWINKLE_NPU_GATED_RMSNorm_FP32` | 将 Gated RMSNorm 强制升到 FP32 计算 | `0`（关闭） |
